...
//! the sand, you might be able to use the glider and the hot air to get all the way up to the
//! metal island!
//!
...
/// zeroes represent differences between the two values above them, this also means there is now a
...
pub fn solve_part1<B: BufRead>(input: B) -> std::io::Result<AdvInt> {
    ...
    for seq in all_seqs {
        ...
        while !seqs.iter().last().unwrap().iter().all(|x| *x == 0) {
